    ...
    computed: {
      ...
      bookData() {
        ...
        if (data && data.length > 0) {
          // 对书本的名字进行预处理，将英文变化为中文
          data.forEach(book => {
            book.text = CATEGORY[book.categoryText.toLowerCase()]
          })
          const number = row * col // 如，row=2，col=3,最多只需要6本书，超过的数据就不要取
          const _bookData = data.slice(0, number) // 当数据data大于number时，我们只需要截取number个数据即可
          const _bookDataRow = [] // 保存我们需要的行数
          let _row = 0
          while (_row < row) {
            // 第一次数据模拟：_row=0,假如列数col为2，我们从0开始截取到第二个元素
            _bookDataRow.push(_bookData.slice(_row * col, _row * col + col))
            _row++
          }
          return _bookDataRow // 返回我们截取的数据
        } else {
          return []
        }
      }
